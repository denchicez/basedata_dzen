...
#    comments = json_stats['comments'] 
    ...
    title = soup.find('h1',class_='article__title article__title_theme_undefined').text #название статьи!!!!
    date_posting = soup.find('span',class_='article-stat__date').text #            время постинга!!!!
    html_text = soup.find('div',class_='article__middle article__middle_theme_undefined')
    ...
    if(len(video_hrefs)==0):
        video_hrefs='Ничего'
    return title,str(tags),date_posting,views,viewsTillEnd,publicationLikeCount,comments_counter,max_path,count_image,str(image_hrefs),count_video,str(video_hrefs),female_counter_like,male_counter_like,interes_top,female_counter_comment,male_counter_comment,len(yandex_dzen),len(set_hrefs_all),simbols_count
if __name__ == '__main__':
    ...
    for url in url_states:
        title,tags,date_posting,views,viewsTillEnd,publicationLikeCount,comments_counter,max_path,count_image,image_hrefs,count_video,video_hrefs,female_counter_like,male_counter_like,interes_top,female_counter_comment,male_counter_comment,yandex_dzen,set_hrefs_all,simbols_count = answer(url)
        sql.execute(f"INSERT INTO states VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(title,tags,date_posting,views,viewsTillEnd,publicationLikeCount,comments_counter,max_path,count_image,image_hrefs,count_video,video_hrefs,female_counter_like,male_counter_like,interes_top,female_counter_comment,male_counter_comment,yandex_dzen,set_hrefs_all,simbols_count))
        db.commit()
